...
def baseline(args, bers, mode):
    ...
    if not os.path.exists(dir):
        os.mkdir(dir)
    np.savez(dir + mode + '_' + str(args.modem_num) + '_' + str(args.qua_bits), snr=SNR, ber=np.array(BER))
    print("%s.npz is saved" % (dir + mode + '_' + str(args.modem_num) + '_' + str(args.qua_bits)))
